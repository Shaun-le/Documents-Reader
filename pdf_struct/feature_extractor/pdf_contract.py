...
class \
        BasePDFFeatureExtractor(BaseFeatureExtractor):
    ...
    @single_input_feature([1])
    def Fontsize(self, tb):
        return tb.Font_size

    def _fontname(self, tb1, tb2):
        if tb1 is None or tb2 is None:
            return False
        if tb1.Font_name == tb2.Font_name:
            return True
        return False

    @pairwise_feature([(0, 1), (1, 2)])
    def Fontname(self, tb1, tb2):
        return self._fontname(tb1, tb2)
