...
from pdfminer.layout import LAParams, LTTextBox, LTTextLine, LTFigure, LTChar, LTText
from pdfminer.pdfdocument import PDFDocument as _PDFDocument
from pdfminer.pdfinterp import PDFResourceManager, PDFPageInterpreter
from pdfminer.pdfpage import PDFPage
from pdfminer.pdfparser import PDFParser
from pdf_struct.core.document import TextBlock
...
class TextBox(TextBlock):
    def __init__(self, text: str, bbox: Tuple[float, float, float, float],
                 blocks: Set[str], page: int, Font_size : float, Font_name : str):
    ...
    def merge_continuous_lines(text_blocks: List['TextBox'], threshold=0.5, space_size=4):
        ...
        while i < (len(text_blocks) - 1):
            ...
            if len(same_line_boxes) > 1:
                ...
                Font_size = same_line_boxes[0].Font_size
                Font_name = same_line_boxes[0].Font_name
                for tbk in same_line_boxes[1:]:
                    ...
                    Font_size = max(Font_size, tbk.Font_size)
                blocks = reduce(operator.or_, (b.blocks for b in same_line_boxes))
                merged_text_blocks.append(TextBox(text, bbox, blocks, tbi.page,Font_size, Font_name))
            else:
...
def parse_layout(layout, page: int, block: str):
    for lt_obj in layout:
        if isinstance(lt_obj, LTTextLine):
            ...
            if len(text.strip()) > 0:
                yield TextBox(text, lt_obj.bbox, {block}, page, Font_size, Font_name)
...
def load_document(path: str, labels: Optional[List[ListAction]], pointers: Optional[List[int]]):
    ...
    Documents.append(text_boxes)
    Documents.append(labels)
    Documents.append(pointers)
    return Documents


def load_from_directory(base_dir: str, annos: AnnoListType):
