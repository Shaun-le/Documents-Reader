...
def to_paragraphs(document, insert_space=True):
    paragraphs = []
    paragraph_levels = []
    last_l, last_p = None, -1
    level = 0
    levels = []
    for l, t, p in zip(document[3], document[1], document[4]):
        ...
        last_l, last_p = l, p
        levels.append(level)
    assert len(levels) == len(document[3])
...
def to_tree(document, insert_space=True):
    sep = ' ' if insert_space else ''
    m = create_hierarchy_matrix(document)
    assert len(m) == len(document[2])
    ...
    for i in range(len(document[2])):
        if document[3][i] == ListAction.ELIMINATE:
            assert not already_added[i]
            text_boxes.append({
                'text': document[2][i].text,
                'siblings': [],
                'ancestors': [],
                'parent': None,
                'descendents': [],
            ...
            text_boxes.append({
                'text': sep.join((document[2][j].text for j in chain([i], continual))),
                'siblings': siblings,
                'ancestors': ancestors,
                'parent': parent,
