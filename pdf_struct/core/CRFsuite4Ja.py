    ...
    for i in range(len(sent)):
        ...
        for y in range(len(d[2])):
            features = {
                ...
                'Fontsize_1_2_3': str(list(list(d[t].values())[8].values())[3][y]),
                'Fontname_1_2': str(list(list(d[t].values())[9].values())[0][y]),
                'Fontname_2_3': str(list(list(d[t].values())[9].values())[1][y]),'''
                'page_like_1': str(list(list(d[t].values())[9].values())[0][y]),
                'page_like_2': str(list(list(d[t].values())[9].values())[1][y]),
                'page_like_3': str(list(list(d[t].values())[9].values())[2][y]),
                'page_like2_1': str(list(list(d[t].values())[10].values())[0][y]),
                'page_like2_2': str(list(list(d[t].values())[10].values())[1][y]),
                'page_like2_3': str(list(list(d[t].values())[10].values())[2][y]),
                'numbered_list_state_value': str(list(list(d[t].values())[12].values())[0][y]),
                'colon_ish_1': str(list(list(d[t].values())[12].values())[0][y]),
                'colon_ish_2': str(list(list(d[t].values())[12].values())[1][y]),
                'punctuated_1': str(list(list(d[t].values())[13].values())[0][y]),
                'punctuated_2': str(list(list(d[t].values())[13].values())[1][y]),
                'mask_continuation_1_2': str(list(list(d[t].values())[14].values())[0][y]),
                'mask_continuation_2_3': str(list(list(d[t].values())[14].values())[1][y]),
                'space_separated_2': str(list(list(d[t].values())[15].values())[0][y]),
                'space_separated_3': str(list(list(d[t].values())[15].values())[1][y]),
                'space_emphasis_1': str(list(list(d[t].values())[16].values())[0][y]),
                'space_emphasis_2': str(list(list(d[t].values())[16].values())[1][y]),
                'parenthesis_emphasis_1': str(list(list(d[t].values())[17].values())[0][y]),
                'parenthesis_emphasis_2': str(list(list(d[t].values())[17].values())[1][y]),
            }
...
def document2features4testptr(feat):
    feats = []
    features = {
...
def document2features4ft(feat):
    ...
    features = {
        ...
        'Fontsize_1_2_3': str(feat['Fontsize']['Fontsize_1_2_3']),
        ...
        'space_emphasis_1': str(feat['space_emphasis']['space_emphasis_1']),
        'space_emphasis_2': str(feat['space_emphasis']['space_emphasis_2']),
        'parenthesis_emphasis_1': str(feat['parenthesis_emphasis']['parenthesis_emphasis_1']),
        'parenthesis_emphasis_2': str(feat['parenthesis_emphasis']['parenthesis_emphasis_2']),
    }
    feats.append(features)
    return features
...
def predict_with_classifiers(clf, clf_ptr, documents, used_features: Optional[List[int]]=None):
    ...
    for document in documents:
        ...
        for i in range(len(d[2])):
            ...
            feats.append([document2features4ft(feat)])
            if used_features is not None:
                feats = feats[:, used_features]
            if d[3][i] != ListAction.ELIMINATE:
                d[3][i] = ListAction(np.array(clf.predict(feats)[0][0],dtype = int))
                ...
                for i in range(j):
                    if d[3][i] == ListAction.DOWN:
                        feat = d[9].extract_pointer_features(
                            d[2], d[3][:j], i, j)
                        X_test_ptr.append(document2features4testptr(feat))
                        ptr_candidates.append(i)
                if len(X_test_ptr) > 0:
                    pointers.append(ptr_candidates[np.argmax(
                        clf_ptr.predict(np.array(X_test_ptr)))])
                else:
