...
class BaseFeatureExtractor(object, metaclass=_FeatureRegisterer):
    ...
    @classmethod
    def append_features_to_document(cls, document):
        feature_extractor = cls(document[2])
        feats_test = feature_extractor.extract_features_all(document[2], None)
        if document[3] is None:
            # prediction
            feats, pointer_feats, pointer_candidates = None, None, None
        else:
            feats = feature_extractor.extract_features_all(document[2], document[3])
            pointer_feats, pointer_candidates = feature_extractor.extract_pointer_features_all(
                document[2], document[3], document[4])
        document.append(feats)
        document.append(feats_test)
        document.append(pointer_feats)
        document.append(pointer_candidates)
        document.append(feature_extractor)
        return document
