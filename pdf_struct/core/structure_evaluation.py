...
def create_hierarchy_matrix(documents) -> np.array:
    ...
    for document in documents:
        m = np.full((len(document[3]) + 1, len(document[3]) + 1), -1, dtype=int)
        m[0, -1] = 1
        last_i = None
        last_l = None
        last_p = None
        for i, (l, p) in enumerate(zip(document[3], document[4])):
...
def evaluate_structure(documents_true, documents_pred):
...
def evaluate_labels(documents_true, documents_pred, confusion_matrix=True):
    ys_pred = [np.array([l.value for l in d[3]]) for d in documents_pred]
    ys_true = [np.array([l.value for l in d[3]]) for d in documents_true]
    if confusion_matrix:
